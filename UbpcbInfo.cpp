...
static char *BpcbInfoToString(const MetaBpcbInfo &mbi, const int *separator)
{
    ...
    snprintf(item, separator[i], "%s", ptr->DisplayName.c_str());
    memcpy(str+nextItem, item, strlen(item));
    nextItem += separator[i];
    i++;
    ...
    switch(mbi.m_iStatus)
    {
        ...
        case ST_STOP:
            sState = "退出态";break;
        default:
            sState = "unknown";break;
    }
    
    snprintf(item,separator[i], "%s", sState.c_str());
    memcpy(str+nextItem, item, strlen(item));
    nextItem += separator[i];
    i++;
    ...
    */
    ...
    LocalDateTime dt(mbi.m_tHeartBeat);
...
//进程管理接口函数
void UbpcbInfo(WINDOW *win, bool bloaddata, bool bshow, bool bactivate, void *data)
{
    if (NULL == win)
    {
        UbpcbDestory();
